...
abstract class StringInterpolator {
  ...
  def interpolateRaw: Tree = interpolate(identity)

  def interpolateString: Tree = interpolate(StringContext.treatEscapes)

  private def interpolate(process: String => String): Tree = c.macroApplication match {
    //case q"$_(..$parts)._(..$args)" =>
    case Applied(Select(Apply(_, parts), _), _, argss) =>
      ...
      if (badlyInvoked) c.macroApplication else myInterpolated(process, parts, args)
    case other =>
      bail(s"Unexpected application ${showRaw(other)}")
      other
  }

  private def makeExpr(stats: List[Tree], expr: Tree): TermTree = {
    val p = c.macroApplication.pos
    Block(stats, atPos(p.focus)(expr)) setPos p.makeTransparent
  }

  private def myInterpolated(process: String => String, parts: List[Tree], args: List[Tree]): TermTree = {
    val constantParts: List[String] = extractConstantPartsAndTreatEscapes(process, parts)

    if (args.isEmpty) {
      // Optimization for case s"" or s"arbitrary string without arguments"
      return makeExpr(Nil, Literal(Constant(constantParts.head)))
    }
    ...
    val allParts = getAllPartsForAppend(constantParts, arguments)

    // code generation

    val plusLenExpr = lengthSum(initialLength, lenNames.flatten.toList)

    val newStringBuilderAndAppendExpr = newStringBuilderWithAppends(q"new java.lang.StringBuilder($plusLenExpr)", allParts.reverse)

    makeExpr(valDeclarations.flatten.toList, q"$newStringBuilderAndAppendExpr.toString")
  }

  private def getAllPartsForAppend(rawParts: Seq[String], arguments: Seq[Tree]) = {
  ...
  private def extractConstantPartsAndTreatEscapes(process: String => String, parts: List[Tree]): List[String] = {
    parts.map {
      case Literal(Constant(x: String)) => process(x)
      case _ => throw new IllegalArgumentException("internal error: argument parts must be a list of string literals")
    }
  }

  private def newStringBuilderWithAppends(newStringBuilderExpr: Tree,
                                          expressions: List[Tree]): Tree = {
    if (expressions.isEmpty) {
      newStringBuilderExpr
    } else {
      Apply(Select(newStringBuilderWithAppends(newStringBuilderExpr, expressions.tail), TermName("append")), List(expressions.head))
    }
  }

  private def lengthSum(const: Int, names: List[TermName]): Tree = {
    if (names.isEmpty) {
      Literal(Constant(const))
    } else {
      Apply(Select(Select(Ident(names.head), TermName("length")), TermName("$plus")), List(lengthSum(const, names.tail)))
    }
  }

  private def interpolated(parts: List[Tree], args: List[Tree]) = {
    ...
    def copyPart(part: Tree, n: Int): Unit = {
      import SpecifierGroups.{Index, Spec}
      val s0 = part match {
  ...
  object Conversion {
    import SpecifierGroups.{CC, Spec}
